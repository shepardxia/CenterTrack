...
from utils.crop_post_process import generic_post_process
from utils.debugger import Debugger
from utils.crop_tracker import Tracker
...
class Detector(object):
    ...
    def crop_tracklets(self, frame, pre_hms, meta):
        ...
        #expand box slightly
        box_scales = box_scales * 1.5 # box expansion ratio
    ...
    def merge_dets(self, dets):
        '''
        Merge outputs from multiple tracking heads.
        '''
        for key in dets.keys():

            dets[key] = dets[key][np.newaxis,:,0]


    def md_iou(self, dets):
        """
        dets - [batch_size x num_anchors x 4]
        """
        ...
        dets_torch = torch.from_numpy(dets['bboxes']).float().to(self.opt.device)

        minx = torch.max(old_boxes[:,:,0], dets_torch[:,:,0])
        maxx = torch.min(old_boxes[:,:,2], dets_torch[:,:,2])
        miny = torch.max(old_boxes[:,:,1], dets_torch[:,:,1])
        maxy = torch.min(old_boxes[:,:,3], dets_torch[:,:,3])
        zeros = torch.zeros(minx.shape).float().to(self.opt.device)
        
        intersection = torch.max(zeros, maxx-minx) * torch.max(zeros,maxy-miny)
        union = area_ap + torch.from_numpy(area_dets).to(self.opt.device) - intersection
    ...
    def run(self, image_or_path_or_tensor, mode=0, meta={}):
        ...
        for scale in self.opt.test_scales:
            ...
            if(mode == 0):
                ...
                output, dets, forward_time = self.process(
                    mode, images, self.pre_images, pre_hms, pre_inds, return_time=True)
                ...
                if self.tracker.tracks != []:
                    crops, pre_hms, box_scales, new_boxes = self.crop_tracklets(images, pre_hms, meta)
                    output, dets, forward_time = self.process(
                    mode, crops, self.pre_images, pre_hms, pre_inds, return_time=True)
                    #self.merge_dets(dets)
        ...
        results = self.merge_outputs(detections, mode)
    ...
    def process(self, mode, images, pre_images=None, pre_hms=None,
        ...
        with torch.no_grad():
            ...
            if(mode == 0):
                ...
                dets = generic_decode(output, K=5, opt=self.opt)
            torch.cuda.synchronize()
            for k in dets:
    ...
    def post_process(self, mode, dets, meta, scale=1):
        dets = generic_post_process(
            mode, self.opt, dets, [meta['c']], [meta['s']],
    ...
    def merge_outputs(self, detections, mode):
        assert len(self.opt.test_scales) == 1, 'multi_scale not supported!'
        results = []
        for i in range(len(detections[0])):
            if detections[0][i]['score'] > self.opt.out_thresh or mode == 1:
